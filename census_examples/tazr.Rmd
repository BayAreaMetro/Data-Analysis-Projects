...
So lets try setting 0.95 as our reasonable ratio and see how close we get to the year 2000 data

```{r}

intersection_df95 <- intersection_df[as.numeric(intersection_df$intersection_ratio)>0.95,]

print(dim(intersection_df95))

print(dim(Tract_zone_2000))

```

Looks like we now have a lot fewer tracts than y2k data. Lets see what ratio gets us to 1405. 

```{r}
intersection_df95 <- intersection_df[as.numeric(intersection_df$intersection_ratio)>0.80,]
print(dim(intersection_df95))
intersection_df95 <- intersection_df[as.numeric(intersection_df$intersection_ratio)>0.75,]
print(dim(intersection_df95))
intersection_df95 <- intersection_df[as.numeric(intersection_df$intersection_ratio)>0.60,]
print(dim(intersection_df95))
intersection_df95 <- intersection_df[as.numeric(intersection_df$intersection_ratio)>0.40,]
print(dim(intersection_df95))
```

Looks like the minimum ratio might be as low as .4, but its probably lower, because this is still a dense matrix. 

We should probably melt the 2000 data back to a dense matrix and check the length and then match that. 

```{r}
dft <- reshape2::melt(Tract_zone_2000, id.vars = "Tract")

print(dim(dft))
```

So we should be looking for a dense matrix with about 7k rows. That makes the overlap ratio much smaller than .4

```{r}
intersection_df_lower <- intersection_df[as.numeric(intersection_df$intersection_ratio)>0.00013,]

print(dim(intersection_df_lower))

intersection_df_lower <- intersection_df[as.numeric(intersection_df$intersection_ratio)>0.00011,]

print(dim(intersection_df_lower))
```

Looks like its between 0.00013 and 0.00011. Of course that assumes around all the same tract matches as well. 
