...
ethnicity_table[ethnicity_table$variable == "B03002_008",]$short_label <- "Other Racial / Ethnic Minority"
```
...
```{r}
ethnicity_table <- ethnicity_table[ethnicity_table$coef_vrt/ethnicity_table$estimate<30,]
...
write_excel_csv(ethnicity_table_wide,"above_average_ethnic_and_racial_groups_by_tract_some_other.csv")

openxlsx::write.xlsx(ethnicity_table_wide,"above_average_ethnic_and_racial_groups_by_tract_some_other.xlsx")
```

Lets also drop those observations with a coefficient of variation that is more than %15 of the estimate

Now we need to spread each geographic unit across a column, since the eventual output/unit of observation is a geography. 

```{r}
ethnicity_table <- ethnicity_table[ethnicity_table$coef_vrt/ethnicity_table$estimate<100,]
ethnicity_table <- ethnicity_table[,c('GEOID','short_label','above_average_flag')]
ethnicity_table <- ethnicity_table[ethnicity_table$above_average_flag==TRUE,]
ethnicity_table <- ethnicity_table %>% 
  distinct(GEOID, short_label, above_average_flag, .keep_all = TRUE)
...
#drop random additional empty column
write_excel_csv(ethnicity_table_wide,"above_average_ethnic_and_racial_groups_by_tract_coeff_var_15.csv")

openxlsx::write.xlsx(ethnicity_table_wide,"above_average_ethnic_and_racial_groups_by_tract_coeff_var_15.xlsx")
```
