...
#### TODO
1.  Add 2014 data in a more systematic way to establish workflow
...
F_OUTPUT_SUM_P_CSV = paste(F_BOX_DIRECTORY, "Typical Weekday Transactions Plaza Statistics.csv", sep = "")
F_OUTPUT_SUM_P_R   = paste(F_BOX_DIRECTORY, "Typical Weekday Transactions Plaza Statistics.Rdata", sep = "")

F_OUTPUT_SUM_D_CSV = paste(F_BOX_DIRECTORY, "Typical Weekday Transactions Designation Statistics.csv", sep = "")
F_OUTPUT_SUM_D_R   = paste(F_BOX_DIRECTORY, "Typical Weekday Transactions Designation Statistics.Rdata", sep = "")
...
# Count the typical weekdays to join with sums
sum.date <- clean.typical %>%
  group_by(year, date, plaza_name, plaza_id, hour_int) %>%
  summarise(count = n()) %>%
  select(-count) %>%
  group_by(year, plaza_name, plaza_id, hour_int) %>%
  summarise(count_typical = n())

# Summary 1: year by plaza by hour
sum.plaza <- clean.typical %>%
  group_by(year, plaza_name, plaza_id, hour_int) %>%
  summarise(sum_transactions = sum(transactions))

sum.plaza <- left_join(sum.plaza, sum.date, by = c("year", "plaza_name", "plaza_id", "hour_int"))

# Summary 2: year by plaza by hour by lane designation
sum.desig <- clean.typical %>%
  group_by(year, plaza_name, plaza_id, hour_int, lane_designation) %>%
  summarise(sum_transactions = sum(transactions))

sum.desig <- left_join(sum.desig, sum.date, by = c("year", "plaza_name", "plaza_id", "hour_int"))
...
#### Write to disk
```{r disk-writes}
save(sum.plaza, file = F_OUTPUT_SUM_P_R)
write.csv(sum.plaza, file = F_OUTPUT_SUM_P_CSV, row.names = FALSE, quote = F)

save(sum.desig, file = F_OUTPUT_SUM_D_R)
write.csv(sum.desig, file = F_OUTPUT_SUM_D_CSV, row.names = FALSE, quote = F)
